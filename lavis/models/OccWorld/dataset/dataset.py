...
class nuScenesSceneDatasetLidar:
    def __init__(
        ...
        if imageset2 is not None:
            with open(imageset2, 'rb') as f:
                data2 = pickle.load(f)

            for key in data:
                for scene in data2[key]:
                    data[key].update({scene: data2[key][scene]})
                    
        self.nusc_infos = data['infos']
        self.scene_names = list(self.nusc_infos.keys())
        print('dataset length:', len(self.scene_names))

        self.over_fitting = over_fitting
        if over_fitting:
            self.scene_names = self.scene_names[:8]
            
            self.nusc_infos = {}
            for sn in self.scene_names:
                self.nusc_infos[sn] = data['infos'][sn]
            # breakpoint()
            print('overfitting dataset length:', len(self.scene_names), len(self.nusc_infos))
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.return_len + self.offset):
            info = self.nusc_infos[scene_name][idx + i]
            token = info['token']


            label_file = os.path.join(self.data_path, f'{self.input_dataset}/{scene_name}/{token}/labels.npz')
            label_file=label_file.replace('gts', 'gts_eval_former') # 'gts_eval_det' gts_eval_fb  gts_eval_former
            label = np.load(label_file)
            occ = label['semantics']
            occs.append(occ)

            # get bbox:
            gt_bboxes_3d.append(info['gt_boxes'])
            gt_names_3d.append(info['gt_names'])

            # get ego
            xys.append(info['gt_ego_fut_trajs'][0])
            # gt_modes.append(info['pose_mode'])

            gt_ego_fut_masks.append(info['gt_ego_fut_masks'][0])
    ...
    def get_image_info(self, scene_name, idx):
        ...
        for cam_type, cam_info in info['cams'].items():
            ...
            if self.over_fitting:  # load file from memory
                if path in self.image_memory.keys():
                    image = self.image_memory[path]
                else:
                    try:
