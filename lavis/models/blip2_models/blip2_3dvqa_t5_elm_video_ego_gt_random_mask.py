...
@registry.register_model("blip2_vqa_t5_elm_video_ego_gt_random_mask")
class Blip2VQAT5ELMVideoEgoGtRandomMask(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def register_vqgan(self, first_stage_config, ego_type='direct_apply'):
        self.init_first_stage_from_ckpt(first_stage_config)
    ...
    def calc_loss_image(self, occ_logits, occ_labels, all_rand_num):
        ...
        occ_logits = occ_logits.contiguous().view(-1, occ_logits.shape[-2], occ_logits.shape[-1])
        occ_labels = occ_labels.contiguous().view(-1, occ_labels.shape[-1])

        occ_logits = occ_logits[all_rand_num]
        occ_labels = occ_labels[all_rand_num]
    ...
    def process_input_split(self, hidden_img_feats, text_tokens, eval_type=None):
        assert hidden_img_feats.shape[1] == text_tokens.shape[1]

        patchified_inputs = []
        patchified_inputs_random_mask = []
        for f in range(hidden_img_feats.shape[1]):
            ...
            patchified_inputs_random_mask.append(patchified_input)

        patchified_inputs_random_mask = torch.stack(patchified_inputs_random_mask, dim=1)

        if self.training:
            patchified_inputs_future = patchified_inputs_random_mask[:, self.mid_time:].contiguous()
            B, F, P, D = patchified_inputs_future.shape
            num_patches = F * P

            all_rand_num = []
            for cur_b in range(B):
                mask_ratio = random.uniform(0.5, 1)
                num_random_mask_patches = int(round(num_patches * mask_ratio))
                random_numbers = random.sample(range(0, num_patches), num_random_mask_patches)
                random_numbers = torch.tensor(random_numbers) + num_patches * cur_b
                all_rand_num.append(random_numbers)

            all_rand_num = torch.concatenate(all_rand_num, dim=0)
            all_rand_num = all_rand_num.view(-1)

            patchified_inputs_future = patchified_inputs_future.view(-1, D)
            pad_embed = self.t5_model.decoder.embed_tokens(torch.tensor([0], device=patchified_inputs_random_mask.device))  # -100: None
            pad_embed = pad_embed.to(patchified_inputs_future.dtype)        
            patchified_inputs_future[all_rand_num] = pad_embed
            patchified_inputs_future = patchified_inputs_future.contiguous().view(B, F, P, D)

            patchified_inputs_random_mask = torch.concatenate((patchified_inputs_random_mask[:, :self.mid_time], patchified_inputs_future), dim=1)

        if eval_type == "iter_refine":
            pad_embed = torch.zeros_like(patchified_inputs_random_mask[:, self.mid_time:])[..., 0]
            pad_embed = self.t5_model.decoder.embed_tokens(pad_embed)  # -100: None
            pad_embed = pad_embed.to(patchified_inputs_random_mask.dtype)  
            patchified_inputs_random_mask = torch.concatenate((patchified_inputs_random_mask[:, :self.mid_time], pad_embed), dim=1)
        ...
        for f in range(hidden_img_feats.shape[1]):
            ...
            patchified_input = patchified_inputs_random_mask[:, f]
        ...
        if self.training:
            return inputs_embeds, patchified_length, all_rand_num
        else:
    ...
    def forward(self, samples):
        ...
        patchified_inputs, patchified_length, all_rand_num = self.process_input_split(hidden_img_feats, text_tokens)
        ...
        with torch.cuda.amp.autocast(dtype=torch.float32):
            ...
            loss_image = self.calc_loss_image(sequence_output_logits, labels, all_rand_num)
    ...
    def post_process(self, z_q, un_flat=False, temperature=1.0, sample=False, top_k=None):
        # inputs: occ -> encode
        ...
        if sample:
            shape = probs.shape
            probs = probs.reshape(shape[0]*shape[1]*shape[2]*shape[3],shape[4])
            ix = torch.multinomial(probs, num_samples=1)
            probs = probs.reshape(shape[0],shape[1],shape[2],shape[3],shape[4])
            ix = ix.reshape(shape[0],shape[1],shape[2],shape[3])
        else:
    ...
    def get_pred(self, logits, dynamic=None, temperature=1.0, sample=False, top_k=None):
        z_q_predict_unpatchify = unpatchify(logits, self.n_patch, self.n_patch, self.n_patch)
        if dynamic is None:
            z_q_predict_unpatchify = self.post_process(z_q_predict_unpatchify, un_flat=True, temperature=temperature, sample=sample, top_k=top_k)
        else:
            z_q_predict_unpatchify = self.post_process_split_dynamic(z_q_predict_unpatchify, un_flat=True, dynamic=dynamic)
        z_q_predict = patchify(z_q_predict_unpatchify, self.n_patch)
        z_q_predict_adapter = self.vqgan_adapter(z_q_predict)  # cnn -> 4096
        return z_q_predict_unpatchify, z_q_predict_adapter
    ...
    #                 cur_quant_z, cur_quant_z_embeds = self.get_pred(lm_logits, dynamic=None, temperature=1.1, sample=True)
    #                 quant_z_list.append(cur_quant_z)
    ...
    #                 ego_pre = text_tokens[:, cur_time:cur_time+1]
    ...
    #     z_q_random_sample = torch.concatenate(quant_z_list, dim=1)
    #     z_q_random_sample = rearrange(z_q_random_sample, 'b (f h1 w1) n1 n2 c -> (b f) c (h1 n1) (w1 n2)', h1=latent_shape[0]//self.n_patch, w1=latent_shape[1]//self.n_patch)

    #     # (b f) c h w
    #     z_q_random_sample = self.first_stage_model.decode(z_q_random_sample)
    #     reconstructed_img = z_q_random_sample
    ...
    def predict_answers(
        ...
        **kwargs,
        ...
        with torch.cuda.amp.autocast(dtype=torch.float32):
            ...
            for cur_time in range(self.mid_time, self.end_time):
                ...
                for _ in range(patchified_length):
                    ...
                    cur_quant_z, cur_quant_z_embeds = self.get_pred(lm_logits, dynamic=None, temperature=4.5, sample=True)
                    quant_z_list.append(cur_quant_z)
                    if self.ego_type == 'pre_fusion':
                        cur_quant_z_embeds = self.pre_fusion(cur_quant_z_embeds, pre_ego_tokens=ego_pre)
                    decoder_inputs_embeds_quant_z = torch.cat([decoder_inputs_embeds_quant_z, cur_quant_z_embeds], dim=1)
        ...
        z_q = torch.stack(quant_z_list, dim=0)
