...
@registry.register_model("blip2_vqa_t5_elm_video_ego_gt_timestep")
class Blip2VQAT5ELMVideoEgoGtTimestep(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def get_tokenized_timesteps(self, tokens, inputs):
        assert self.t5_tokenizer is not None, "Error tokenizer is None!"
        chunks = []
        for token in tokens:
            chunk = self.t5_tokenizer(token).input_ids  
            # print(chunk)
            chunk = torch.tensor(chunk[0], dtype=torch.long, device=inputs.device)  # chunk[1:3]
            chunk = self.t5_model.encoder.embed_tokens(chunk)
            chunk = chunk.unsqueeze(0).expand(inputs.shape[0], -1, -1)
            chunks.append(chunk)
        return chunks   
    ...
    def split_occ_and_pose(self, inputs, input_length, pose_length, timestep_length, chunk_length, split=False):
        """Splits the inputs into occupancy and pose components."""
        total_length = input_length + pose_length + timestep_length + 2 * chunk_length
        ...
        if split:
            ...
            occ_list, ego_list = [], []
            for inp in inputs:
                occ_list.append(inp[:, timestep_length:timestep_length+input_length])
                ego_list.append(inp[:, timestep_length+input_length:timestep_length+input_length+pose_length])
    ...
    def process_input_split(self, hidden_img_feats, text_tokens):
        ...
        for f in range(hidden_img_feats.shape[1]):
            ...
            patchified_input = torch.cat((self.timestep_chunk[f], self.image_start_chunk, patchified_input, ego_tokens, self.image_end_chunk), dim=1)
    ...
    def forward(self, samples):
        ...
        if self.timestep_chunk is None:
            chunks = self.get_tokenized_timesteps(['1', '2', '3', '4', '5', '6'], img_feats)
            self.timestep_chunk = chunks
        ...
        with torch.cuda.amp.autocast(dtype=torch.float32):
            ...
            sequence_output_logits, ego_logits = self.split_occ_and_pose(
                sequence_output, patchified_length, 1, 1, self.image_start_chunk.shape[1], split=False)
        ...
        with torch.cuda.amp.autocast(dtype=torch.float32):
            ...
            for cur_time in range(self.mid_time, self.end_time):
                if cur_time == self.mid_time:
                    decoder_inputs_embeds_quant_z = torch.cat((self.timestep_chunk[cur_time], self.image_start_chunk), dim=1)
                else:
                    decoder_inputs_embeds_quant_z = torch.cat((decoder_inputs_embeds_quant_z, self.timestep_chunk[cur_time], self.image_start_chunk), dim=1)  # add new start
