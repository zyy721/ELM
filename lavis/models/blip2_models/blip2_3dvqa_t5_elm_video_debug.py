...
@registry.register_model("blip2_vqa_t5_elm_video_debug")
class Blip2VQAT5ELMVideoDebug(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def register_vqgan(self, first_stage_config):
        ...
        # self.vqgan_adapter = nn.Sequential(nn.Linear(256*16*16, 128),
        #                                    nn.Linear(128, self.t5_model.config.hidden_size),)
    ...
    #         targets_hold_place = torch.ones_like(targets)[:, 0:1]
    ...
    #         lm_logits = lm_logits.view(B, 24*24, 1024)
    #         loss = None
