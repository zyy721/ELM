...
from lavis.models.OccWorld.model.VAE.vae_2d_resnet import VAERes2D
from lavis.models.OccWorld.model.VAE.quantizer import VectorQuantizer
...
from lavis.models.OccWorld.dataset import get_dataset, get_nuScenes_label_name
from lavis.models.OccWorld.dataset.metric_util import multi_step_MeanIou
import numpy as np
import copy
...
@registry.register_model("blip2_vqa_t5_elm_video_occ")
class Blip2VQAT5ELMVideoOcc(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def __init__(
        ...
        self.occ_start_chunk = None
        self.occ_end_chunk = None
    ...
    @ torch.no_grad()
    def encode_occ(self, occ):
        bs, f, h, w, d = occ.shape
        occ, shape = self.occ_vae.forward_encoder(occ)
        occ = self.occ_vae.vqvae.quant_conv(occ)
        occ, _, (_, _, min_encoding_indices) = self.occ_vae.vqvae.forward_quantizer(
            occ, None, False, False, False)
        occ = rearrange(occ, '(b f) c h w -> b f h w c', b=bs)
        
        # label patchify
        # min_encoding_indices = rearrange(
        #     min_encoding_indices, 
        #     '(b f) (h1 n1) (w1 n2) -> b (f h1 w1) (n1 n2)', 
        #     b=bs, n1=self.n_patch_occ, n2=self.n_patch_occ
        # )
        ...
        return occ, min_encoding_indices, shape
    ...
    def calc_loss_occ(self, occ_logits, occ_labels):
        occ_logits = rearrange(occ_logits, 'b fhw (nn c) -> b fhw nn c', nn=self.n_patch_occ*self.n_patch_occ)
        # assert occ_logits.shape[-1] == self.occ_size, f"Error, occ logits size {occ_logits.shape} and vocab size {self.occ_size} mismatch!"
    ...
    def process_input_split_image(self, inputs_embeds, text_tokens):
        assert inputs_embeds.shape[1] == text_tokens.shape[1]

        patchified_inputs = []
        for f in range(inputs_embeds.shape[1]):
            patchified_input = patchify(inputs_embeds[:, f:f+1], self.n_patch)
    ...
    def process_input_split_occ(self, inputs_embeds, text_tokens):
        assert inputs_embeds.shape[1] == text_tokens.shape[1]

        patchified_inputs = []
        for f in range(inputs_embeds.shape[1]):
            patchified_input = patchify(inputs_embeds[:, f:f+1], self.n_patch_occ)
            patchified_length = patchified_input.shape[1]
            patchified_input = self.occ_adapter(patchified_input)

            if self.ego_type == 'place_holder':
                ego_tokens = torch.zeros_like(self.occ_end_chunk) - 100
            elif self.ego_type == 'direct_apply':
                ego_tokens = self.ego_adapter(text_tokens[:, f:f+1])
            elif self.ego_type == 'pre_fusion':
                ego_tokens = self.ego_adapter(text_tokens[:, f:f+1])
            ...
            patchified_input = torch.cat((self.occ_start_chunk, patchified_input, ego_tokens, self.occ_end_chunk), dim=1)
            patchified_inputs.append(patchified_input)
    ...
    def forward(self, samples):
        ...
        text_tokens_mask = samples["text_tokens_mask"]

        B = input_ids.shape[0]
        device = input_ids.device

        # get chunks
        if self.occ_start_chunk is None or self.occ_end_chunk is None:
            chunks = self.get_tokenized_chunks([OCC_START, OCC_END], input_ids)
            self.occ_start_chunk = chunks[0]
            self.occ_end_chunk = chunks[1]

        hidden_occ_feats, labels, _ = self.encode_occ(input_ids)
        patchified_inputs, patchified_length = self.process_input_split_occ(hidden_occ_feats, text_tokens)
        ...
        with torch.cuda.amp.autocast(dtype=torch.float32):
            ...
            sequence_output_logits, ego_logits = self.split_occ_and_pose(
                sequence_output, patchified_length, 1, self.occ_start_chunk.shape[1], split=False)

            labels_pred = labels[:, self.mid_time:].contiguous()
            text_tokens_pred = text_tokens[:, self.mid_time:].contiguous()
            text_tokens_pred_mask = text_tokens_mask[:, self.mid_time:].contiguous()

            sequence_output_logits = self.occ_head(sequence_output_logits)
            ego_logits = self.lm_head(ego_logits)

            loss_image = self.calc_loss_occ(sequence_output_logits, labels_pred)
            loss_ego = self.calc_loss_ego(ego_logits, text_tokens_pred, text_tokens_pred_mask)
    ...
    @ torch.no_grad()
    def decode_occ(self, z_q_predict, shape1, shape2):  # z_q_predict: (b f) c h w
        assert len(z_q_predict.shape) == 4, "z_q_predict shape mismatch"
        # z_q_predict = rearrange(z_q_predict, 'b (f c) (h w) -> (b f) c h w', c=128, h=50, w=50)

        z_q_predict = self.occ_vae.vqvae.post_quant_conv(z_q_predict)
        z_q_predict = self.occ_vae.forward_decoder(z_q_predict, shape1, shape2)
        
        return z_q_predict.argmax(dim=-1).detach()   
    ...
    def post_process_occ(self, z_q, un_flat=False):
        # inputs: occ -> encode
        # labels: occ -> encode -> min_indices
        # logits: predicted occ
        # new logits: logits -> argmax -> 1 index -> codebook -> 2 occ code -> 3 occ adapter
    ...
    def get_pred_occ(self, logits, dynamic=None):
        z_q_predict = unpatchify(logits, self.n_patch_occ, self.n_patch_occ, self.n_patch_occ)
        if dynamic is None:
            z_q_predict = self.post_process_occ(z_q_predict, un_flat=True)
        else:
            z_q_predict = self.post_process_split_dynamic(z_q_predict, un_flat=True, dynamic=dynamic)
        z_q_predict = patchify(z_q_predict, self.n_patch_occ)
        z_q_predict = self.occ_adapter(z_q_predict)  # cnn -> 4096
        return z_q_predict
    ...
    def get_pred_output_occ(self, occ_logits, latent_shape, shape1, shape2):
        z_q = unpatchify(occ_logits, self.n_patch_occ, latent_shape[0], latent_shape[1])
        z_q = self.post_process_occ(z_q, un_flat=False)
        z_q = self.decode_occ(z_q, shape1, shape2)
    ...
    def predict_answers(
        ...
        **kwargs,
        ...
        shape2 = input_ids[:, self.mid_time:].shape  # shape2: for occ vae decoding

        hidden_occ_feats, _, shape_occ = self.encode_occ(input_ids)
        latent_shape = hidden_occ_feats.shape[2:4]  # get original latent h and w
        hidden_occ_feats = hidden_occ_feats[:, :self.mid_time]
        
        patchified_inputs, patchified_length = self.process_input_split_occ(hidden_occ_feats, text_tokens[:, :self.mid_time])
        ...
        with torch.cuda.amp.autocast(dtype=torch.float32):
            ...
            for cur_time in range(self.mid_time, self.end_time):
                if cur_time == self.mid_time:
                    decoder_inputs_embeds_quant_z = self.occ_start_chunk
                else:
                    decoder_inputs_embeds_quant_z = torch.cat((decoder_inputs_embeds_quant_z, self.occ_start_chunk), dim=1)  # add new start
                ...
                for _ in range(patchified_length):
                    ...
                    lm_logits = self.occ_head(sequence_output[:, -1:])
                    ...
                    cur_quant_z_embeds = self.get_pred_occ(lm_logits, dynamic=None)
                ...
                decoder_inputs_embeds_quant_z = torch.cat((decoder_inputs_embeds_quant_z, ego_tokens, self.occ_end_chunk), dim=1)  # add new end
        ...
        z_q = self.get_pred_output_occ(sequence_output_logits, latent_shape, shape_occ, shape2).detach()

        results = {"occ": z_q, "ego_pose": ego_logits, "ego_label": text_tokens[:, self.mid_time:]}

        assert self.iou is not None and self.miou is not None, "iou and miou implemented error!"
        occ, occ_label = results['occ'], input_ids[:, self.mid_time:]
        self.calc_metrics(occ, occ_label)

        ego, ego_label = results['ego_pose'], results['ego_label']
        ego_L2 = ((ego - ego_label) ** 2).sum(dim=2).sqrt()
        # breakpoint()
        # if self.only_calc_metrics:
        #     return CausalLMOutputWithPast(
        #         loss=None,
        #         logits={"ego_L2": ego_L2}
        #     )
        
        results.update({"ego_L2": ego_L2})
    ...
    def from_config(cls, cfg):
        ...
        occ_config = cfg.get("occ_config")
        model.register_occ_world(occ_config)
        model.occ_vae.requires_grad_(False)    

        # load config
        cfg = Config.fromfile(occ_config)
        # register iou and miou metrics
        label_name = get_nuScenes_label_name(cfg.label_mapping)
        unique_label = np.asarray(cfg.unique_label)
        unique_label_str = [label_name[l] for l in unique_label]

        times = 3
        CalMeanIou_sem = multi_step_MeanIou(unique_label, cfg.get('ignore_label', -100), unique_label_str, 'sem', times=times)
        CalMeanIou_vox = multi_step_MeanIou([1], cfg.get('ignore_label', -100), ['occupied'], 'vox', times=times)

        model.register_metrics(CalMeanIou_sem=CalMeanIou_sem, CalMeanIou_vox=CalMeanIou_vox)   


        return model

    def register_occ_world(self, occ_config):
        # load config
        cfg = Config.fromfile(occ_config)
        occ_autoencoder = MODELS.build(cfg.model)
        occ_autoencoder.init_weights()
        ckpt = torch.load(cfg.load_from, map_location='cpu')
        if 'state_dict' in ckpt:
            state_dict = ckpt['state_dict']
        else:
            state_dict = ckpt
        # occ_autoencoder.load_state_dict(state_dict, strict=True)
        occ_autoencoder.load_state_dict(state_dict, strict=False)

        occ_autoencoder.eval()

        occ_vae = occ_autoencoder
        self.occ_vae = occ_vae
        vqvae = occ_vae.vqvae
        occ_size = vqvae.n_e
        latent_size = vqvae.e_dim
        self.occ_size = occ_size

        self.occ_adapter = nn.Linear(latent_size*self.n_patch_occ*self.n_patch_occ, self.t5_model.config.hidden_size)
        self.occ_head = nn.Linear(self.t5_model.config.hidden_size, self.n_patch_occ*self.n_patch_occ*self.occ_size)

    def register_metrics(self, CalMeanIou_sem, CalMeanIou_vox):
        self.iou = CalMeanIou_vox
        self.miou = CalMeanIou_sem

        self.iou.reset()
        self.miou.reset()

    def calc_metrics(self, occ, occ_label):
        occ_iou = copy.deepcopy(occ)
        occ_iou[occ_iou != 17] = 1
        occ_iou[occ_iou == 17] = 0

        occ_label_iou = copy.deepcopy(occ_label)
        occ_label_iou[occ_label_iou != 17] = 1
        occ_label_iou[occ_label_iou == 17] = 0

        self.miou._after_step(occ, occ_label)
        self.iou._after_step(occ_iou, occ_label_iou)